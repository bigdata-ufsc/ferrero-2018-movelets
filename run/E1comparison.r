...
DIR_PATH <- "../results/ACMSAC2018/E1/"
...
results$fmeasure <- round(results$fmeasure,2)
results$Accuracy <- round(results$Accuracy,2)

dt <- results

dt <- subset(results, select = c("algorithm","dataset","method","fmeasure"))
...
#dt.test <- dt.wide[,c("TB-RB","TCPR","Dodge","Zheng","Xiao","Movelets")]
